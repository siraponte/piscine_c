...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/11 12:00:50 by cserapon          #+#    #+#             */
/*   Updated: 2020/12/12 07:50:29 by cserapon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_convert_base(char *nbr, char *base_from, char *base_to)
{
	...
	if (!(ft_is_base_valid(base_from, 0, 0) > 1
				&& ft_is_base_valid(base_to, 0, 0) > 1))
		return (0);
	decimal = ft_to_decimal(nbr, base_from, ft_is_base_valid(base_from, 0, 0));
	if (!(tab = (char *)malloc(ft_numlen(decimal,
						ft_is_base_valid(base_to, 0, 0), 0)
					+ (decimal > 0 ? 0 : 1) * sizeof(char))))
		return (0);
	index = ft_to_new_base(decimal, base_to, tab, 0);
	if (decimal < 0)
	{
		tab[ft_numlen(decimal, ft_is_base_valid(base_to, 0, 0), 0) + 1] = '-';
		ft_rev_char_tab(tab, ft_numlen(decimal,
					ft_is_base_valid(base_to, 0, 0), 0) + 2);
	}
	else
		ft_rev_char_tab(tab, ft_numlen(decimal,
					ft_is_base_valid(base_to, 0, 0), 0));
